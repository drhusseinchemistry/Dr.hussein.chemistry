...
const PollSection: React.FC<PollSectionProps> = ({ studentName: initialName }) => {
  ...
  useEffect(() => {
    // Get all visible polls without orderBy to avoid excluding polls without 'order' field
  ...
  useEffect(() => {
    ...
    const checkSubmission = async () => {
      if (!studentName && anyPollRequiresName) return;
  ...
  return (
    <div className="mt-12 pt-10 border-t-2 border-gray-50">
      ...
      <div className="space-y-10">
        {hasSubmitted ? (
          <div className="text-center py-16 space-y-6 bg-green-50/50 rounded-3xl border-2 border-dashed border-green-100">
            <div className="w-20 h-20 bg-white text-green-500 rounded-full flex items-center justify-center mx-auto shadow-xl">
              <CheckCircle2 className="w-10 h-10" />
            </div>
            <div>
              <h4 className="text-2xl font-black text-gray-800 mb-2">سوپاس بۆ تە!</h4>
              <p className="text-gray-500 font-bold">بۆچوونێن تە ب سەرکەفتی گەهشتنە مە.</p>
            </div>
          </div>
        ) : (
          <div className="bg-white p-8 rounded-3xl border-2 border-indigo-50 shadow-sm space-y-10">
            {needsNameInput && (
              <div className="bg-indigo-50 p-6 rounded-2xl border-2 border-indigo-100">
                ...
                <input 
                  ...
                  className="w-full p-4 rounded-xl border-2 border-white focus:border-indigo-500 outline-none font-bold shadow-sm text-lg"
                />
              </div>
            )}
            ...
            <div className="space-y-12">
              {polls.map((poll, idx) => (
                <div key={poll.id} className="animate-in fade-in slide-in-from-bottom-2">
                  ...
                  <div className="px-2">
                    {poll.type === 'CHOICE' ? (
                      ...
                      <textarea 
                        ...
                        className="w-full p-5 rounded-2xl border-2 border-gray-100 focus:border-indigo-500 outline-none font-bold text-lg min-h-[120px] resize-none bg-gray-50/30"
                      />
                    )}
                  </div>
                </div>
